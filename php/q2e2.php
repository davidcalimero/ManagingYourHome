										...
										<table id="currentLocation">
											<tr>
												...
												<td id="divisionTitle"><span id="path">Planta ► <?php 
														require 'procedures/connection.php';
														$query = "select dNome from divisao NATURAL JOIN equipada where eID = 'q2e2';";
														$result = pg_query($query) or die(pg_last_error());
														foreach (pg_fetch_assoc($result) as $value)
															$nome = $value;
														echo $value;
												...
												<span id="location"><?php 
														require 'procedures/connection.php';
														$query = "select eNome from equipamento where eID = 'q2e2';";
														$result = pg_query($query) or die(pg_last_error());
														foreach (pg_fetch_assoc($result) as $value)
															$nome = $value;
														echo $value;
												...
												<div style="position: relative; max-width: 60%; margin-top: -30%;">
													<div id="blind_feedback">
														<?php 
															...
															$query = "select v1 from equipamento where eID = 'q2e2';";
															$result = pg_query($query) or die(pg_last_error());
															foreach (pg_fetch_assoc($result) as $value)
																$valor = $value;
															pg_free_result($result);
											...
											<td width="50%">
												<div id="blind_controls">
													...
													<?php 
															...
															onmouseup=\"updateBlindDB('q2e2')\"></div><div id=\"closed\">Fechado</div>";
													?>
								...
								<form name="changeForm">
									<table class="editTable" border="0" width="100%">
										...
										<tr><td colspan="2" class="loginButtons">
											<input type="button" name="alterarDivisao" value="Guardar" id="saveAD" class="loginButtons" onclick="saveEdit('q2e2','utiliza','eID');"/> 
											<input type="reset" name="cancelar" value="Repor" id="cancelAD" class="loginButtons" onclick="cancelEdit('q2e2','utiliza','eID');"/>
										</td></tr>
									</table>
								</form>
							</div>
